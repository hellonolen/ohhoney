...
export default function LandingPage() {
  return (
    <>
      ...
      {/* ── FOOTER ── */}
      <footer style={{ background: 'var(--black)', borderTop: '1px solid rgba(255,255,255,0.08)' }}>
        <div className="container" style={{ padding: '48px 40px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 40, marginBottom: 48 }}>
            <div>
              <p className="label" style={{ color: 'rgba(255,255,255,0.35)', marginBottom: 20 }}>Intelligence</p>
              {['Today', 'Inbox', 'Calendar', 'Notes', 'Tasks'].map(l => (
                <div key={l} style={{ marginBottom: 12 }}>
                  <Link href={`/dashboard/${l.toLowerCase()}`} style={{
                    fontSize: 'var(--text-12)', fontWeight: 300, color: 'rgba(255,255,255,0.55)',
            ...
            <div>
              <p className="label" style={{ color: 'rgba(255,255,255,0.35)', marginBottom: 20 }}>Membership</p>
              {['Apply', 'Pricing', 'For Teams', 'Enterprise'].map(l => (
                <div key={l} style={{ marginBottom: 12 }}>
                  <Link href={`/${l.toLowerCase().replace(' ', '-')}`} style={{
                    fontSize: 'var(--text-12)', fontWeight: 300, color: 'rgba(255,255,255,0.55)',
                    transition: 'color var(--ease)',
                  }}
                    onMouseEnter={e => { (e.target as HTMLElement).style.color = 'var(--white)'; }}
            ...
            <div>
              <p className="label" style={{ color: 'rgba(255,255,255,0.35)', marginBottom: 20 }}>Company</p>
              {['Journal', 'About', 'Privacy', 'Terms'].map(l => (
                <div key={l} style={{ marginBottom: 12 }}>
            ...
            <div>
              <p className="label" style={{ color: 'rgba(255,255,255,0.35)', marginBottom: 20 }}>Contact</p>
              <p style={{ fontSize: 'var(--text-12)', fontWeight: 300, color: 'rgba(255,255,255,0.55)', lineHeight: 1.7 }}>
                hello@ohhoney.ai<br />
                Members: support@ohhoney.ai
              </p>
            </div>
          </div>
          <hr className="rule" style={{ borderColor: 'rgba(255,255,255,0.08)', marginBottom: 32 }} />
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <p className="label" style={{ color: 'rgba(255,255,255,0.2)', letterSpacing: '0.08em' }}>OhHoney</p>
            <p style={{ fontSize: 11, fontWeight: 300, color: 'rgba(255,255,255,0.2)', letterSpacing: '0.06em' }}>
